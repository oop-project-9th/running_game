...
class ExampleWorld(
    ...
    private var blinkTimer = 0f// 점수 증가용 타이머
    ...
    override fun update(delta: Float) {
        blinkTimer += delta
        when (state) {
            GameState.MENU      -> updateMenu()       // 추가
            GameState.IN_PLAY   -> updateInPlay(delta)
            GameState.GAME_OVER -> updateGameOver()
        }
    }

    /** MENU: ENTER 키를 누르면 게임 시작 */
    private fun updateMenu() {
        if (InputHandler.isKeyJustPressed(com.badlogic.gdx.Input.Keys.ENTER)) {
    ...
    private fun updateInPlay(delta: Float) {
        ...
        //   기존 바로 게임오버 로직에서 HP 차감 및 사망 조건부 게임오버
    ...
    private fun updateGameOver() {
        if (InputHandler.isKeyJustPressed(com.badlogic.gdx.Input.Keys.R)) {
            // 재시작: 플레이어·적 교체
            remove(player)
            remove(enemy)
            player = ExamplePlayer(100f, groundY, worldWidth, worldHeight, groundY)
            enemy  = ExampleEnemy(100f, worldHeight - 100f, 0f, worldWidth)
            add(player)
            add(enemy)
            state = GameState.IN_PLAY
            score = 0
    ...
    override fun render(delta: Float) {
        ...
        when (state) {
            ...
            GameState.IN_PLAY   -> drawHud()
            GameState.GAME_OVER -> { drawHud(); drawGameOverOverlay() }
        }
    }
